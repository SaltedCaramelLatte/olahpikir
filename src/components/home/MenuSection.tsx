...
const MenuSection = () => {
    ...
    const [visibleItems, setVisibleItems] = useState(Array(list.length).fill(false));
        ...
        <section className="relative flex flex-col items-center justify-center min-h-screen bg-light-background dark:bg-dark-background py-10 px-4 sm:px-6 lg:px-8">
            ...
            <div className="gap-6 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4">
                {list.map((item, index) => {
                    ...
                    return (
                        <div
                            className={`menu-item transition-opacity duration-1000 ${visibleItems[index] ? 'opacity-100' : 'opacity-0'}`}
                            data-index={index}
                            key={index}
                        >
                            <Card
